    ...
    while 1:
        try:
            ...
            break
                ...
                #y = y.split ("-")
                #y = int(y[0])
                #y = int (fecha.strftime ('%d')) - y
                #print (y)
                #input ()
                #if Matrix[linea-1][y][turno - 1] == 0:
                    #break
        except ValueError:
            print('ERROR EN EL PROGRAMA SOLO PUEDES INTRODUCIR NUMEROS, NO LETRAS')
    fecha = datetime.now()
...
def consultaParciales (linea,turno,dia):
    ...
    txtProd1 = "Productos: " + str(x[0]) + "\n"
    txtDet1 = "Paradas: " + str(x[1]) + "\n"
    nuevo = txtLin1 + txtTurn1 + txtProd1 + txtDet1
    print (nuevo)
...
while True:
    ...
    if op == 1:
        ...
        while 1:
            try:
                linea__ = validacionConRango('Ingresa el numero de linea para consulta parcial (1 - 5): ',1,5)
                turno__ = validacionConRango("Ingresa el turno para consulta parcial (1.- Mañana 2.- Tarde 3.- Noche): ",1,3)
                dia__ = validacionConRango("Ingresa el numero del dia de la semana (Lunes = 1, Martes = 2...): ",1,7)
                dia__ = dia__ - 1
                break
            except ValueError:
                print('ERROR EN EL PROGRAMA SOLO PUEDES INTRODUCIR NUMEROS, NO LETRAS')
        consultaParciales(linea__,turno__,dia__)
